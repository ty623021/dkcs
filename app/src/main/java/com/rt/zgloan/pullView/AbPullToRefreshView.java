 ...
 */
package com.rt.zgloan.pullView;
...
public class AbPullToRefreshView extends LinearLayout {
    ...
    private void initContentAdapterView() {
        ...
        if (nestedScrollView == null && mAdapterView == null && mScrollView == null && recyclerView == null) {
    ...
    private boolean isRefreshViewScroll(int deltaY) {
        ...
        if (recyclerView != null) {
            ...
            if (deltaY > 0) {
                ...
                if (child == null) {
                    // 如果recyclerView中没有数据,不拦截
                    //return false;

                    mPullState = PULL_DOWN_STATE;
                    return true;
                }
                if (layoutManager.findFirstVisibleItemPosition() == 0 && child.getTop() == 0) {
                    mPullState = PULL_DOWN_STATE;
                    return true;
                }
                int top = child.getTop();
                int padding = recyclerView.getPaddingTop();
                if (layoutManager.findFirstVisibleItemPosition() == 0 && Math.abs(top - padding) <= 11) {
                    mPullState = PULL_DOWN_STATE;
                    return true;
                }
            } else if (deltaY < 0) {
                ...
                // 最后一个子view的Bottom小于父View的高度说明recyclerView的数据没有填满父view,
                // 等于父View的高度说明mAdapterView已经滑动到最后
                if (lastChild.getBottom() <= getHeight() && layoutManager.findLastVisibleItemPosition() == layoutManager.getItemCount() - 1) {
                    mPullState = PULL_UP_STATE;
                    return true;
                }
            }
    ...
    public void onFooterLoadFinish() {
        ...
        if (recyclerView != null) {
            LinearLayoutManager layoutManager = (LinearLayoutManager) recyclerView.getLayoutManager();
            int countNew = layoutManager.getItemCount();
            //判断有没有更多数据了
            if (countNew > mCount) {
                mFooterView.setState(AbListViewFooter.STATE_READY);
            } else {
